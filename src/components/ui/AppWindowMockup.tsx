...
const publicTables = ["customers", "events", "invoices", "order_coupons", "order_items", "orders", "products"];

export function AppWindowMockup({ className }: { className?: string }) {
  return (
    <div
      className={cn("overflow-hidden border border-border bg-[#1e1e1c] shadow-2xl", "shadow-accent-glow", className)}
    >
      {/* Title bar */}
      <div className="flex items-center gap-2 border-border border-b bg-[#161614] px-3 py-2.5">
        ...
        <div className="ml-4 flex gap-px">
          <div className="bg-[#2a2a26] px-3 py-1 font-mono text-[10px] text-text-muted">invoices</div>
          <div className="border-[#2a2a26] border-x border-t bg-[#1e1e1c] px-3 py-1 font-mono text-[#e8e4dc] text-[10px]">
            orders
          </div>
          <div className="flex items-center px-2 text-[10px] text-text-muted">+</div>
        </div>
      </div>
      ...
      <div className="flex">
        ...
        <div className="hidden w-44 shrink-0 border-border border-r bg-[#161614] sm:block">
          ...
          <div className="border-border border-b px-3 py-2">
            <p className="font-mono text-[9px] text-text-muted uppercase tracking-widest">Connections</p>
            <div className="mt-1.5 flex items-center gap-1.5">
              <div className="h-1.5 w-1.5 rounded-full bg-[#44ff88]" />
              <span className="font-mono text-[10px] text-text-secondary">dev</span>
              <span className="ml-auto font-mono text-[9px] text-text-muted">2</span>
            </div>
            <div className="mt-0.5 ml-3">
              <p className="truncate font-mono text-[9px] text-text-muted">slashtable-dev-ecommerce</p>
            </div>
          </div>

          {/* Explorer */}
          <div className="px-3 py-2">
            <p className="font-mono text-[9px] text-text-muted uppercase tracking-widest">Explorer</p>
            <div className="mt-1.5 border border-border px-2 py-1">
              <span className="font-mono text-[9px] text-text-muted">Filter tables, views...</span>
            </div>
            <div className="mt-2 space-y-0.5">
              {sidebarTables.map((t) => (
                <div key={t.name}>
                  <div className="flex items-center gap-1">
                    <span className="text-[9px] text-text-muted">{t.expanded ? "▼" : "▶"}</span>
                    <span className="font-mono text-[10px] text-text-secondary">{t.name}</span>
                    <span className="ml-auto font-mono text-[9px] text-text-muted">{t.count}</span>
                  </div>
                  {t.expanded && (
                      ...
                      {publicTables.map((table) => (
                        <p
                          ...
                          className={cn(
                            ...
                            table === "orders" ? "bg-[#2a2a26] text-[#e8e4dc]" : "text-text-muted",
                          )}
                        >
                          {table}
                        </p>
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="flex items-center gap-1 overflow-x-auto border-border border-b px-3 py-1.5">
            ...
            <span className="font-mono text-[10px] text-text-muted">orders</span>
            <span className="text-[10px] text-text-muted">→</span>
            <span className="font-mono text-[10px] text-accent">customers #6e07...</span>
            <span className="text-[10px] text-text-muted">→</span>
            <span className="font-mono text-[#e8e4dc] text-[10px]">orders</span>
            <span className="ml-auto whitespace-nowrap font-mono text-[9px] text-text-muted">312# order_items</span>
          </div>

          {/* Filter bar */}
          <div className="flex items-center gap-2 border-border border-b px-3 py-1">
            <span className="font-mono text-[10px] text-text-secondary">customer_id</span>
            <span className="font-mono text-[10px] text-text-muted">=</span>
            <span className="truncate font-mono text-[10px] text-text-secondary">6e0700c6-214a-471c-8989</span>
            <span className="ml-auto cursor-pointer font-mono text-[9px] text-accent">Clear</span>
          </div>
          ...
          <div className="overflow-hidden">
            ...
            <div className="flex border-border border-b bg-[#161614]">
              {columns.map((col) => (
                ...
                >
                  <span className="text-text-muted/60">{col.type}</span> {col.name}
                </div>
              ))}
            </div>
            ...
            {/* Rows */}
            {rows.map((row, i) => (
              <div key={row.id} className={cn("flex border-border/50 border-b", i === 0 && "bg-[#2a2a26]/50")}>
                ...
                <div className="w-44 shrink-0 border-border/50 border-r px-2 py-0.5 font-mono text-[10px]">
                  <span className="text-accent">{row.customer_id}</span>
                ...
                <div className="w-32 shrink-0 px-2 py-0.5 font-mono text-[10px] text-text-muted">{row.notes}</div>
              </div>
            ))}
          </div>
          ...
          <div className="flex items-center justify-between border-border border-t bg-[#161614] px-3 py-1">
            <span className="font-mono text-[9px] text-text-muted">slashtable-dev-ecommerce</span>
            <span className="font-mono text-[9px] text-text-muted">4ms · 100 loaded / ~2.0K total</span>
          </div>
        </div>
      </div>
    </div>
