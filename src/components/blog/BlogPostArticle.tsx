...
export function BlogPostArticle({ post }: { post: BlogPost }) {
  return (
    <article>
      ...
      <h1 className="font-display text-4xl text-text">{post.title}</h1>
      <p className="mt-4 text-sm text-text-secondary leading-relaxed">{post.description}</p>
      ...
      <section className="mt-10 border border-border border-dashed bg-surface/50 p-5" aria-labelledby="tldr-heading">
        ...
        <p className="mt-3 text-sm text-text-secondary leading-relaxed">{post.tldr}</p>
      </section>
